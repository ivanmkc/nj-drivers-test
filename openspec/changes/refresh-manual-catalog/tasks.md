...
- [ ] 7.1 ~~Run `translate.py <code> ja` for every state added in Waves 1-3.~~ **Removed:** JA is no longer in scope (preference recorded 2026-04-29). Existing per-wave `es` translations in Tasks 4.2/5.2/6.4 cover all required locales for new states.

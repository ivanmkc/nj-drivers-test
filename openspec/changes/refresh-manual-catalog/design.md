...
- Translating every new state to es/ja. Each wave's translation work is best-effort — en is required, es is high-value for most states, ja is a nice-to-have.
...
**5. Translations: en required per wave; es batched at end of each wave; ja deferred to a separate sweep at the end.**

Running translations inline per state burns Gemini budget. Better: complete en for the whole wave, then run a bulk `translate.py --all-pending es`, then `... --all-pending ja`. Catches translation failures in one place.
